...
// TestOpenCartUpdateIsGuarded covers the three conditions on moving a shop order, since
// moving the wrong one is worse than moving none: the order must be settled, the match
// must not be a guess, and the reference must be a shop order id rather than a B2B
// portal UID.
func TestOpenCartUpdateIsGuarded(t *testing.T) {
	tests := []struct {
		name string
		ref  string
		want bool
	}{
		{"shop order id", "17216", true},
		{"b2b order uid", "b5fea01edea5a09e7d9c74d3e81e537d", false},
		{"crm style id", "ORD-1234", false},
		{"empty", "", false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := isNumericRef(tt.ref); got != tt.want {
				t.Errorf("isNumericRef(%q) = %v, want %v", tt.ref, got, tt.want)
			}
		})
	}
}

// TestOpenCartUpdateSkippedWhenUnconfigured checks the default: with no status id set,
// order statuses are left alone rather than moved to status zero.
func TestOpenCartUpdateSkippedWhenUnconfigured(t *testing.T) {
	db := newFakeMatchDB()
	db.byNumber["PROF 700/2026"] = doc("PROF 700/2026", "17216", "PLN", 100.00)
	db.unmatched = []*entity.BankTransaction{credit("k1", "PROF 700/2026", "CLIENT", 10000, "PLN")}

	c := testMatcher(db) // ocPaidStatus stays zero, oc stays nil
	if _, err := c.MatchBankPayments(context.Background()); err != nil {
		t.Fatal(err)
	}
	if fact := db.facts["17216"]; fact == nil || !fact.Settled() {
