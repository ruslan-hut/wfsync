...
//
// VAT MOSS details (vat_moss_details, nested in the invoice payload):
...
type Invoice struct {
	...
	VatMossDetails []*VatMossDetailLine    `json:"vat_moss_details,omitempty" bson:"vat_moss_details,omitempty"` // OSS evidence, nested in invoice
...
// VatMossDetail holds OSS evidence for an invoice, proving the buyer's country.
// Nested in the invoice payload for EU B2C invoices with destination-country VAT rates.
// Two non-contradictory pieces of evidence are required by EU regulation.
type VatMossDetail struct {
	Type                 string `json:"type" bson:"type"`                                   // service code: "BA" (goods/WSTO), "SA"-"SE" (services), "TA"-"TK" (telecom)
	Evidence1Type        string `json:"evidence1_type" bson:"evidence1_type"`               // "A" (address), "B" (IP), "C" (bank), "D" (SIM), "E" (landline), "F" (other)
	Evidence1Description string `json:"evidence1_description" bson:"evidence1_description"` // free text describing the evidence
	Evidence2Type        string `json:"evidence2_type" bson:"evidence2_type"`               // second evidence type (must differ from first)
	Evidence2Description string `json:"evidence2_description" bson:"evidence2_description"` // free text describing the second evidence
}

// VatMossDetailLine wraps VatMossDetail for the wFirma API array structure.
// The API expects: "vat_moss_details": [{"vat_moss_detail": {...}}, ...]
type VatMossDetailLine struct {
